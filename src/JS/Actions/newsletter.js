...
export const subNewsletter = (newNewsLetter,setEmail) => async (dispatch) => {
  ...
  try {
    let result = await axios.post(
      "https://demofcs.onrender.com/api/newsletter/addNewsLetter",
      newNewsLetter
    );
...
export const getNewsLetters = () => async (dispatch) => {
  ...
  try {
    ...
    let result = await axios.get("https://demofcs.onrender.com/api/newsletter/list-newsletter", config);
...
export const deleteNewsletter = (id) => async (dispatch) => {
  ...
  try {
    ...
    let result = await axios.delete(`https://demofcs.onrender.com/api/newsletter/${id}`, config);
    window.scrollTo({
      top: 0,
      behavior: "smooth",
    });
