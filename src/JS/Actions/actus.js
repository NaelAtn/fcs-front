...
export const addActus = (actus, navigate) => async (dispatch) => {
  ...
  try {
    ...
    let result = await axios.post("https://demofcs.onrender.com/api/actus/postActus", actus, config);
...
export const getOneActus = (id) => async (dispatch) => {
  dispatch({ type: LOAD_ACTUS });
  try {
    let result = await axios.get(`https://demofcs.onrender.com/api/actus/getoneactus/${id}`);
...
export const getActus = () => async (dispatch) => {
  dispatch({ type: LOAD_ACTUS });
  try {
    let result = await axios.get("https://demofcs.onrender.com/api/actus/getActus");
...
export const deleteActus = (id) => async (dispatch) => {
  ...
  try {
    ...
    let result = await axios.delete(`https://demofcs.onrender.com/api/actus/delete/${id}`, config);
    window.scrollTo({
      top: 0,
      behavior: "smooth",
    });
...
export const addFavActus = (id) => async (dispatch) => {
  dispatch({ type: LOAD_ACTUS });
  try {
    await axios.put(`https://demofcs.onrender.com/api/actus/addFav/${id}`);
    dispatch(getActus());
...
export const removeFavActus = (id) => async (dispatch) => {
  dispatch({ type: LOAD_ACTUS });
  try {
    await axios.put(`https://demofcs.onrender.com/api/actus/removeFav/${id}`);
    dispatch(getActus());
    dispatch(getOneActus());
  } catch (error) {
    dispatch({ type: FAIL_ACTUS, payload: error.response.data.errors });
